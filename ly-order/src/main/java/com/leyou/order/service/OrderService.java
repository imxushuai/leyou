...
public class OrderService {
    ...
    public Long createOrder(OrderDto orderDto) {
        ...
        if (addressDTO == null) {
            // 商品不存在，抛出异常
            throw new LyException(LyExceptionEnum.RECEIVER_ADDRESS_NOT_FOUND);
        }
        ...
        ArrayList<Long> ids = new ArrayList<>(skuNumMap.keySet());
        List<Sku> skus = new ArrayList<>();
        ids.forEach(id -> skus.add(goodsClient.querySkuById(id)));
...
//        goodsClient.decreaseStock(orderDto.getCarts());
    ...
    public String generateUrl(Long orderId) {
        ...
        if (order.getOrderStatus().getStatus() != OrderStatusEnum.INIT.value()) {
            throw new LyException(LyExceptionEnum.ORDER_STATUS_EXCEPTION);
        }

        //todo 这里传入一份钱，用于测试使用，实际中使用订单中的实付金额
        String url = payHelper.createPayUrl(orderId, "乐优商城测试", /*order.getActualPay()*/1L);
        if (StringUtils.isBlank(url)) {
            throw new LyException(LyExceptionEnum.CREATE_PAY_URL_ERROR);
        }
    ...
    public Order queryById(Long orderId) {
        ...
        if (order == null) {
            throw new LyException(LyExceptionEnum.ORDER_NOT_FOUND);
        }
        OrderDetail orderDetail = new OrderDetail();
        orderDetail.setOrderId(orderId);
    ...
    public PageResult queryOrderByPage(Integer page, Integer rows) {
